...
const TypedEntry = ({ content, type, onComplete }) => {
  ...
  const styles = {
    ...
    system: "text-orange-300/90 font-bold",
    response: "text-white/90 terminal-glow",
    error: "text-gray-400 font-bold italic",
    header: "text-gray-200 font-black tracking-widest uppercase mb-2 border-b border-gray-500/20 pb-1",
    info: "text-gray-300 font-medium",
...
const TerminalCLI = () => {
  ...
  const [history, setHistory] = useState([
    { type: 'system', content: '>>> ANNE_OS v4.0.0 :: INITIALIZING CORE...' },
  ...
  const COMMANDS = useMemo(() => {
    ...
    const registry = {
      help: {
        desc: 'Show all available commands', run: () => [
      ...
      whoami: {
        desc: 'Show full profile dossier', run: () => [
      ...
      ls: {
        desc: 'List directory components', run: () => [
      ...
      skills: {
        desc: 'Show categorized tech stack', run: () => [
      ...
      project: {
        desc: 'Show project specs [id]', run: (args) => {
          const id = args[0];
      ...
      edu: {
        desc: 'Show education credentials', run: () => [
      ...
      exp: {
        desc: 'Show professional experience', run: () => [
      ...
      socials: {
        desc: 'List linked social nodes', run: () => [
      ...
      pwd: { desc: 'Print working directory', run: () => [{ type: 'secondary', content: '/users/anne/portfolio' }] },
      goto: { desc: 'Navigate to [section]', run: runGoto },
      cd: { desc: 'Alias for goto', run: runGoto },
      got: { desc: 'Alias for goto', run: runGoto },
      search: {
        desc: 'Query portfolio database', run: (args) => {
          ...
          >
            ...
            <div className="relative z-10 flex items-center justify-between border-b border-gray-400/20 bg-gray-400/5 px-5 py-3">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => setIsOpen(false)} className="text-white/40 hover:text-orange-400 transition-colors">
